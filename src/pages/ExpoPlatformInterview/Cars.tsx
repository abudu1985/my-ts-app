...
type TypedCar<ColorType,T = undefined> = {
  id: number;
  number: string;
  description: string;
  color: ColorType;
...
const carsList: TypedCar<number>[] = [
  {
    id: 1,
    number: "AH0001AA",
    color: 1,
...
const Cars: FC = () => {
  const readyList = carsList.reduce((acc, car) => {
    ...
    return [...acc, { ...car, color: findedColor?.color, type: findedType }] as TypedCar<string, string>[];
  }, [] as TypedCar<string, string>[]);
